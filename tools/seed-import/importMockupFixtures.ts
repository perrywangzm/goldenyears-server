...
import { importMockupData, type MockupFixtures } from "../../src/db/seeds/importMockupData";
...
export async function loadMockupFixtures(): Promise<MockupFixtures> {
  const [facilities, reviews, careTypes, features, languages, regions] = await Promise.all([
...
async function main() {
  ...
  if (!connectionString) {
    throw new Error("DATABASE_URL or HYPERDRIVE_CONNECTION_STRING is required to seed Postgres.");
  }
